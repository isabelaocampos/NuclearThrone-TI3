...
public abstract class Entity extends Rectangle {
    ...
    public boolean intersects(Entity other) {
        ...
        if ((this instanceof Player || this instanceof Enemy) && (other instanceof Bullet || other instanceof Enemy)) {
            return false;
        }
    ...
    public static boolean isOutOfScreen(Entity e) {
        return e.getX() + e.getWidth() + 10 > MainController.getWidth() || e.getX() <= 0
                || e.getY() + 30 + e.getHeight() > MainController.getHeight() || e.getY() <= 0;
    }

    public static Direction getSideOut(Entity e) {
        if (e.getX() + e.getWidth() + 10 > MainController.getWidth()) {
            return Direction.RIGHT;
        }
        if (e.getX() <= 0) {
            return Direction.LEFT;
        }
        if (e.getY() + 30 + e.getHeight() > MainController.getHeight()) {
